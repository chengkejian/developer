...
//   const inputLabel = React.useRef(null);
  ...
  const handleChangeSilder = (event, newValue) => {
    setValues(oldValues => ({
      ...oldValues,
      slider_range: newValue,
    }));
  };
function format_slider_value(value) {
//  console.log("---"+values.slider_range)
...
//    alert("values="+JSON.stringify(values))
    ...
    if(values.file === ''){
        ...
        if(values.slider_range){
            url += "&block_range="+values.slider_range[0]+","+values.slider_range[1];
        }
        window.open(url, 'log');
    }
  };
